for((i=1;i<5;i++)) do
	ruby ../record_to_head.rb -s ~/scratch/nips/data/JPEGImages/ -r ~/scratch/nips/scan_voc_8pix.txt --corenode ~/git/posecpp/model/head_clusters_46.txt -n ~/git/posecpp/model/nips_clusters.txt -t ~/git/posecpp/model/nips_transforms.txt --anchor-transform ~/git/posecpp/model/nips_head_transforms.txt -o ~/scratch/nips/torso_voc_8pix_nips_complex_$i.txt --complex  --bias --group_threshold $i 
