...
lcrecords = Hash[Record.seperate_records(src, des, IO.foreach(lcdat)).reject { |r| r.rects.nil? }.each { |r| r.pick_good_set head; r.group_rects table }.reject { |r| r.groups.values.to_set.empty? }.map { |r| [r.filename, r] }]
