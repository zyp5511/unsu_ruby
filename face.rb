...
#pred_dat = Hash[Record::seperate_records(src,des,IO.foreach(pred_fn)).select{|r|r.rects!=nil}.each{|r|r.pick_good_set head;r.group_rects_with_graph  nettable,table}.select{|r|r.groups.values.to_set.length>0}.map{|r|[r.filename, r]}] 
pred_dat = Hash[Record::seperate_records(src,des,IO.foreach(pred_fn)).select{|r|r.rects!=nil}.each{|r|r.pick_good_set head;r.group_rects table}.select{|r|r.groups.values.to_set.length>0}.map{|r|[r.filename, r]}] 
