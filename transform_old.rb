...
class LCTransformSet
  ...
  def simplify(appr, id = 594) # rule extraction using different lambda
    @transforms.group_by(&:from).flat_map do |_kf, fg|
      fg.group_by(&:to).select { |k, _v| k == id }.map do |_kt, tg|
        sorted_tg = tg.sort
...
class LCTransformTable
  ...
  def self.loadTable(fname, src, des)
    ...
    b = Hash[trans.map { |k, v| [k, v.min_by(&:reliability)] }]
