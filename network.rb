...
class Network
  ...
  def subgraph_quality(nodeset)
    tot = 0
    nodeset.each do |n|
      next if @distances[n].nil?
      @distances[n].each do |k, d|
        tot += d if nodeset.include?(k)
      end
    end
    tot
  end
  ...
  def restrict(fname)
    ...
    IO.foreach(fname) do |line|
      ...
      @nodes << good[0]
      @nodes << good[1]
      @good_set << if good[0] < good[1]
                     (good[0] * 10_000 + good[1])
                   else
                     (good[1] * 10_000 + good[0])
                   end
  ...
  def self.loadTable(fname)
    ...
    IO.foreach(fname) do |line|
      ...
      trans[rule.from * 10_000 + rule.to] = rule
      trans[rule.to * 10_000 + rule.from] = rule.inv
    end
    Network.new(trans)
  end
end
